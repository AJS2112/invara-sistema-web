...
class Cnf_monedas extends CI_Controller {
	protected $headers;

	public function __construct(){
		parent:: __construct();
		$this->load->helper('authjwt_helper');
		$this->load->model("cnf_monedas_m");
	}
	...
	public function getList($idEmpresa){
		...
		if (!$auth_user){
			...
			$menu=$this->cnf_monedas_m->getList($idEmpresa);
	...
	public function getOne($idEmpresa,$id=NULL){
		...
		if (!$auth_user){
			...
			if ($id){
				$data=$this->cnf_monedas_m->getOne($id);
			} else {
				$data=array(
					"id"=>0,
					"order_id"=>0,
		...
		if (!$auth_user){
			...
			if ($this->input->post()){
				...
				$data=array(
					...
					'es_inactiva'=>$this->input->post("es_inactiva",true),
				);

				if ($id!=0){
					$lastId=$this->cnf_monedas_m->update($data);
				}else{
					$lastId=$this->cnf_monedas_m->insert($data);		
				}
