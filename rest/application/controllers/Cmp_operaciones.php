	...
	public function setOne(){
		...
		if (!$auth_user){
			...
			if ($this->input->post()){
				...
				//REGISTRO NUEVO
				if ($id=="0"){
					$ultimo=$this->cmp_operaciones_m->getLast($head["id_empresa"],$head["id_tipo_operacion"]);
					$lastnumero=$ultimo["lastnumero"]+1;
					$lastcontrol=str_pad($lastnumero, 8, '0', STR_PAD_LEFT);
					...
					$x=$this->cmp_operaciones_m->update($data);	
					$lastId=$id;	
				}
				...
				if ($lastId){
					//PROCESA DETALLE
					if ($id!="0"){
						$delDetail=$this->inv_movimientos_m->delete($id);	
						$this->_reversarInventario($detail, $tipo["signo_inventario"]);													
					}
