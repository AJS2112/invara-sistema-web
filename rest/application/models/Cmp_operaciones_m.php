<?php
class Cmp_operaciones_m extends CI_Model{
    ...
    public function getList($idEmpresa,$idTipo){        
        $query = $this->db->query("CALL compras_operaciones_SEL(?,?)", array('id_empresa'=>$idEmpresa, 'id_tipo'=>$idTipo));
        mysqli_next_result($this->db->conn_id);
        return $query->result_array();
    }

    public function getOne($id=NULL){  
        $query = $this->db->query("CALL compras_operaciones_ONE(?)", array('id'=>$id));
        mysqli_next_result($this->db->conn_id);
        if ($query->num_rows()===1){
    ...
    public function getLast($idEmpresa=NULL, $idTipo=NULL){  
        $query = $this->db->query("CALL compras_operaciones_LAST(?,?)", array('id_empresa'=>$idEmpresa, 'id_tipo'=>$idTipo));
        mysqli_next_result($this->db->conn_id);
        if ($query->num_rows()===1){
            return $query->row_array();
        }
